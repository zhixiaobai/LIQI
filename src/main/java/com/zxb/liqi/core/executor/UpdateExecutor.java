 ...
 * @Description
 */
public class UpdateExecutor implements BaseExecutor {
    private final BaseParser parser;
    private final Class<?> entityClass;
    private final PreparedStatement preparedStatement;

    public UpdateExecutor(BaseParser parser, Class<?> entityClass, PreparedStatement preparedStatement) {
    ...
    public Object executor() {
        try {
            ...
            if (mysqlAffectedRows == 0) {
                throw new SQLException("The affected rows is zero, the update operation failure");
            }
